...
>>> array = rasterio.open('img/agé.bmp').read()
...
>>> r = requests.post(endpoint, files={'file': open('img/agé.bmp', 'rb')})
...
>>> rasterio.open('img/11-band.tif').profile  #doctest:+NORMALIZE_WHITESPACE
...
>>> r = requests.post(endpoint, files={'file': open('img/11-band.tif', 'rb')})
...
>>> rasterio.open('img/redding1.tiff').profile  #doctest:+NORMALIZE_WHITESPACE
...
>>> r = requests.post(endpoint, files={'file': open('img/redding1.tiff', 'rb')})
...   #doctest:+SKIP
